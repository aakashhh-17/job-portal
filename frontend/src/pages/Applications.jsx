...
const Applications = () => {
  ...
  const updateResume = async ()=>{
    try {
      if(!resume) return toast.error('upload resume first');
  ...
  return (
    <>
      <Navbar />
      <div className='p-4 container min-h-[65vh] 2xl:px-20 mx-auto my-10'>
            ...
            isEdit || userData && userData.resume === "" ? (
              <>
                <label className='flex items-center' htmlFor="resumeUpload">
                  <a className='bg-blue-100 text-blue-600 px-4 py-2 rounded-lg mr-2'>{resume ? resume.name  : 'Select Resume'}</a>
              ...
              <div className='flex gap-2 '>
                <a target='_blank' href={userData.resume} className='bg-blue-100 text-blue-600 px-4 py-2 rounded-lg' >Resume
                </a>
        ...
        <table className='min-w-full bg-white border border-gray-200 rounded-lg '>
          ...
          <tbody>
            {userApplications.map((job, index) => true ? (
              <tr key={index}>
                <td className='px-4 py-3 flex items-center gap-2 border border-gray-200'>
                  <img className='size-8' src={job.companyId.image} alt="" />
                  {job.companyId.name}
                </td>
                <td className='py-2 px-4 border border-gray-200'>{job.jobId.title}</td>
                <td className='py-2 px-4 border border-gray-200 max-sm:hidden'>{job.jobId.location}</td>
                <td className='py-2 px-4 border border-gray-200 max-sm:hidden'>{moment(job.date).format('ll')}</td>
